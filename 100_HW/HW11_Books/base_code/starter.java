...
class starter {
		...
		*/
		while (true) {
			...
			if (choice.equals("file")) {
				System.out.println("Currently using file: " + textFile);
			}
			...
			if (choice.equals("change")) {
				System.out.println("Enter the new file name (must exist in 'books' folder):");
	...
	*/
	public static void printsNumber(String[] arr, int num) {
		if (num > arr.length) {
			System.out.println("Number is too biggg! Try again.");
		}
		if (num <= arr.length) {
			for (int i = 0; i < num; i++) {
				System.out.println(arr[i] + " ");
